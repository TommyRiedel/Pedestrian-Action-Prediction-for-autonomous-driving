"""Training the model on pedestrian action prediction on the combined (JAAD and PIE) dataset with Pytorch Lightning (Tensorboard for logging metrics)
"""
import torch
...
class LitMyModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_nb):
        # Training step logic: x = keypoint location, y = label, bb = BBox location, vel = veloctiy (not for JAAD -> also not for combined dataset)
        x = batch[0]
        y = batch[1]
        if self.bbox:
            bb = batch[2]
    ...
    def validation_step(self, batch, batch_nb):
        # Validation step logic: x = keypoint location, y = label, bb = BBox location, vel = veloctiy (not for JAAD -> also not for combined dataset)
        x = batch[0]
        y = batch[1]
        if self.bbox:
            bb = batch[2]
    ...
    def test_step(self, batch, batch_nb):
        # Test step logic: x = keypoint location, y = label, bb = BBox location, vel = veloctiy (not for JAAD -> also not for combined dataset)
        x = batch[0]
        y = batch[1]
        if self.bbox:
            bb = batch[2]
    ...
    def predict_step(self, batch, batch_idx):
        # Prediction step logic: x = keypoint location, y = label, bb = BBox location, vel = veloctiy (not for JAAD --> also not for combined dataset)
        x = batch[0]
        y = batch[1]
        if self.bbox:
            bb = batch[2]
...
def main(args):
    ...
    # Load train, validation and test DataSets (PIE)
    tr_data_pie = DataSet(
        path=args.data_path_pie,
        data_type="pie",
        data_set="train",
        balance=False,
        bbox=args.bbox,
    ...
    # Combine JAAD and PIE datasets to COMB dataset
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        monitor="val_acc",
        save_top_k=5,
        filename="comb-{epoch:02d}-{val_acc:.3f}",
        mode="max",
...
if __name__ == "__main__":
    ...
    # path to directoy with data (JAAD)
    parser.add_argument(
        "--data_path_jaad",
        type=str,
        default="./",
        help="Path to the train and test data (JAAD)",
    )
    # path to directoy with data (PIE)
    parser.add_argument(
        "--data_path_pie",
        type=str,
        default="./",
        help="Path to the train and test data (PIE)",
    )
    # specify batch size
    parser.add_argument(
