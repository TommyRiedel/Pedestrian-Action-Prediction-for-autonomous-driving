...
def main(args):
    ...
    # Load Datasets
    tr_data = DataSet(path=args.data_path, data_set='train', balance=False, bbox=args.bbox, velocity=args.velocity)
    te_data = DataSet(path=args.data_path, data_set='test', balance=False, bbox=args.bbox, velocity=args.velocity)
    val_data = DataSet(path=args.data_path, data_set='val', balance=False, bbox=args.bbox, velocity=args.velocity)
    ...
    logger = TensorBoardLogger("tb_logs", name="final_pie")
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=args.logdir, monitor='val_acc', save_last=True,
        filename='pie-{epoch:02d}-{val_acc:.3f}', save_weights_only=True)
    ...
    #torch.save(mymodel.model.state_dict(), args.logdir + 'pie_case_pvb.pth')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--logdir', type=str, default="./data/pie/tb/final/", help="logger directory for tensorboard")
    parser.add_argument('--epochs', type=int, default=175, help="Number of eposch to train")
    parser.add_argument('--lr', type=int, default=5e-5, help='learning rate to train')
    parser.add_argument('--data_path', type=str, default='./', help='Path to the train and test data')
    ...
    parser.add_argument('--velocity', type=bool, default=True, help='activate the use of the gps velocity')
